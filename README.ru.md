...
## ⚡ Два уровня команд: Глобальные Скиллы vs Проектные Воркфлоу

Для максимальной прозрачности и удобства команды в Antigravity разделены на два уровня:

```text
┌─────────────────────────────────────────────────────────────────────────────────────────────┐
│                       ДВУХУРОВНЕВАЯ СИСТЕМА КОМАНД В ANTIGRAVITY                            │
├─────────────────────────────────────────────────────────────────────────────────────────────┤
│ 1. ГЛОБАЛЬНЫЕ СКИЛЛЫ (Доступны ВСЕГДА и ОВТОВСЮДУ)                                          │
│    Работают в любом окне, пустом диалоге или сторонней папке (~/.gemini/config/skills/)     │
│    • < > new-project      ➔ Создание нового проекта с полным пайплайном по одному названию  │
│    • < > adopt-project    ➔ Безопасный онбординг существующего кода без мутации файлов      │
│    • < > companion-pack   ➔ Сборка чистого архитектурного ZIP для ChatGPT / Claude          │
│    • < > stitch-sync      ➔ Экспорт экранов и токенов дизайна в Google Stitch               │
├─────────────────────────────────────────────────────────────────────────────────────────────┤
│ 2. ПРОЕКТНЫЕ ВОРКФЛОУ (Работают ТОЛЬКО внутри проекта под пайплайном)                       │
│    Берутся из каталога .agents/workflows/ открытого проекта и опираются на .agy/            │
│    • /nextphase           ➔ Автономное исполнение поступившего Action Packet                │
│    • /auditphase          ➔ Независимый аудит критериев приёмки и точности метрик          │
│    • /fixcritical         ➔ Изолированное исправление дефектов из FINDINGS.json             │
│    • /fastpatch           ➔ Быстрый точечный патч для правок в 1–3 строки                   │
│    • /shipcheck           ➔ Предрелизная проверка целостности и готовности к поставке       │
└─────────────────────────────────────────────────────────────────────────────────────────────┘
```

| Уровень | Где доступен | Где лежит физически | Как выглядит в меню | Примеры |
|---|---|---|---|---|
| **Глобальные Скиллы** | Везде (пустой чат, любая папка) | `~/.gemini/config/skills/` | `< > имя-скилла` | `/new-project`, `/adopt-project`, `/companion-pack`, `/stitch-sync` |
| **Проектные Воркфлоу** | Только внутри инициализированного проекта | `<проект>/.agents/workflows/` | Имя воркфлоу в меню | `/nextphase`, `/auditphase`, `/fixcritical`, `/fastpatch`, `/shipcheck` |
...
### ⚡ Управление автоматическими хэндофами (фильтрация диалогов conversations.txt):
Для предотвращения избыточных выгрузок и перехвата буфера обмена архивы `LATEST_CONTEXT.zip` формируются динамически только для явно разрешённых диалогов.
- **Файл конфигурации:** `C:\Scripts\AntigravityProjects\companion-handoff\conversations.txt`
- **Как включить диалог в белый список:** вставьте его Conversation ID (например, `4e568acf-a6a6-48f1-8b08-789a4210a93b`) с новой строки:
  ```text
  c0f55ce7-2989-4557-8cb0-e1f7980033e3   # Huawei Health export
  4e568acf-a6a6-48f1-8b08-789a4210a93b   # H10 Athlete Cardio Lab
  ```
- **Как временно отключить диалог:** закомментируйте строку символом `#` (например, `# c0f55ce7-...`).
- **Как разрешить все диалоги:** укажите отдельной строкой символ звёздочки `*`.
- **Мгновенный Hot-Reload:** скрипт хука перечитывает `conversations.txt` «на лету» при каждой паузе агента. Никаких команд перезапуска выполнять не нужно — настройки применяются сразу после сохранения файла.
...
|:---|:---:|:---:|:---|:---|
| **`/new-project`** | 🚀 Старт | 🌐 Глобальный | Создание нового проекта под ключ по одному названию | `/new-project MyProject` |
| **`/adopt-project`** | 🚀 Старт | 🌐 Глобальный | Подключение существующего репозитория без изменения старого кода | `/adopt-project C:\repo` |
| **`/companion-pack`** | 📦 Контекст | 🌐 Глобальный | Сборка чистого архитектурного кода для ChatGPT/Claude в 1 МБ ZIP | `/companion-pack` |
| **`/nextphase`** | ⚙️ Исполнение | 📁 Проектный | Автономное выполнение утвержденной задачи Action Packet | `/nextphase` или *«делай»* |
| **`/goal`** | 🧠 Режим | 💡 Модификатор | Модификатор сверхупорства: работать без пауз до 100% верификации | `/nextphase /goal` |
| **`/auditphase`** | 🔍 Аудит | 📁 Проектный | Независимый аудит критериев приёмки и точности метрик | `/auditphase` |
| **`/fixcritical`** | 🔧 Ремонт | 📁 Проектный | Изолированное исправление дефектов, найденных аудитом | `/fixcritical` |
| **`/fastpatch`** | ⚡ Патч | 📁 Проектный | Быстрое точечное исправление (1–3 строки, опечатки, конфиги) | `/fastpatch` |
| **`/stitch-sync`** | 🎨 UI/UX | 🌐 Глобальный | Экспорт экранов и токенов дизайна в интерактивный Google Stitch | `/stitch-sync` |
| **`/interview-me`** | 💡 Мышление | 🌐 Глобальный | Инженерный опрос по одному вопросу за раз для кристаллизации идеи | *«проведи интервью»* |
