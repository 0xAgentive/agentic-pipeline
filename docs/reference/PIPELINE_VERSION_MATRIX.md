...
|---|---:|---|
| Package release | **1.2.4** | Governance & Routing Stabilization: objective routing, independent acceptance, inventory trust, installation identity and cross-platform runtime checks. |
| Previous stable package | 1.2.3 | Distribution Integrity: state profiles, fresh-install smoke, command inventory, template hygiene, leakage checks and tracked-only release packages. |
| Canonical playbook | **1.2.0** | `docs/AGENTIC_PIPELINE_PLAYBOOK.md` and the versioned/reference copies remain byte-identical. |
| Runtime | **1.2.1** | Runtime handshake `1.1.0`, project-local inventory authority, explicit compatibility and fail-closed route derivation. |
