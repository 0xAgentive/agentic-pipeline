...
def test_T48_runtime_target_baseline():
    ...
    with open(baseline_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    assert data.get("ecosystem_version") == "1.2.11"
    assert data.get("pipeline_package") == "1.2.11"
    assert data.get("runtime") == "1.2.11"
    assert data.get("companion") == "1.2.11"
...
def test_T52_release_attestation_matches_files():
    ...
    with open(config_path, 'r', encoding='utf-8') as f:
        config = json.load(f)
    assert baseline.get("ecosystem_version") == "1.2.11"
    assert config.get("ecosystem_version") == "1.2.11" and config.get("version") == "4.3.4"
