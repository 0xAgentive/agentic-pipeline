...
function Test-PackageRoot {
  ...
  foreach ($Required in @($ManifestPath, $VersionPath, $AttestationPath, (Join-Path $Root 'Update-AgenticContextHandoff-v1.2.14.ps1'))) {
...
$IntegrationRoot = Join-Path $Root 'integrations\companion-handoff-1.2.14'
$SourceRoot = Join-Path $IntegrationRoot 'source'
$UpdaterPath = Join-Path $IntegrationRoot 'Update-AgenticContextHandoff-v1.2.14.ps1'
...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
  $OutputRoot = Join-Path $Root '.artifacts\release-kit\1.2.14\context-handoff'
}
...
try {
  ...
  Copy-Item -LiteralPath $UpdaterPath -Destination (Join-Path $Pack 'Update-AgenticContextHandoff-v1.2.14.ps1')
