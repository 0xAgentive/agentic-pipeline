...
function Test-PackageIntegrity {
  ...
  foreach ($Name in @('VERSION.json', 'MANIFEST.json', 'SOURCE_ATTESTATION.json', 'Update-AgenticContextHandoff-v1.2.21.ps1', 'source')) {
...
if ($HookCommand.Length -gt $HookCommandMaxLength) {
  throw (
    "Generated Context Handoff hook command length $($HookCommand.Length) exceeds " +
    "the fail-closed Windows command limit $HookCommandMaxLength. No installation writes were performed."
  )
}
