<#
.SYNOPSIS
    Update-AntigravityCompanionHandoff.ps1 - Updater for Antigravity Companion Handoff v1.2.26
.DESCRIPTION
...
Write-Host "Updating Antigravity Companion Handoff v1.2.26" -ForegroundColor Cyan
Write-Host "==============================================" -ForegroundColor Cyan
...
Write-Host "Target version: 1.2.26" -ForegroundColor Green

if ($currentVersion -eq "1.2.26" -and -not $Force) {
    Write-Host "Already at v1.2.26. Use -Force to reinstall." -ForegroundColor Yellow
    return
}
...
if (-not $DryRun) {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "`nUpdate to v1.2.26 completed successfully!" -ForegroundColor Green
    } else {
