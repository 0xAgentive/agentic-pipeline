...
Run `Build-AgenticContextHandoffPackage-v1.2.9.ps1` from a clean exact release commit. The ZIP contains:
...
Extract the downloaded release asset and run `Update-AgenticContextHandoff-v1.2.9.ps1` from that extracted directory. The default run is read-only and returns a plan; add `-Apply` to commit it.
