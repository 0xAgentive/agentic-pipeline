...
if ([string]($Version.package_version) -ne '1.2.6') { throw 'Local repository is not version 1.2.6.' }
...
if ($TagLines.Count -gt 0) {
  ...
  if ($LocalTag) {
    ...
    Invoke-Native -FilePath git -ArgumentList @('-C',$Repo,'tag','-a',$Tag,'-m','Agentic Pipeline 1.2.6 - Autonomous Audit Convergence') | Out-Null
  }
...
$ReleaseZip = Join-Path $ArtifactRoot 'pipeline\agentic-pipeline-1.2.6.zip'
$CompanionZip = Join-Path $ArtifactRoot 'companion\agentic_pipeline_companion_pack1.2.4.zip'
$RuntimeZip = Join-Path $ArtifactRoot 'runtime\agentic-project-runtime-1.2.3-overlay.zip'
foreach ($Artifact in @($ReleaseZip,$CompanionZip,$RuntimeZip)) {
...
$Notes = Join-Path $ArtifactRoot 'RELEASE_NOTES_1.2.6.md'
@'
# Agentic Pipeline 1.2.6 - Autonomous Audit Convergence
...
- bounded grouped repair batches;
...
- Companion 1.2.4 with compact finding/repair delta output;
- standalone Antigravity runtime 1.2.3 overlay.
...
if ($Existing.Code -ne 0) {
  Invoke-Native -FilePath $Gh -ArgumentList @(
    ...
    '--title','Agentic Pipeline 1.2.6 - Autonomous Audit Convergence',
    '--notes-file',$Notes,
    $ReleaseZip,$CompanionZip,$RuntimeZip
  ) | Out-Null
}
else {
  $UploadArguments = @('release','upload',$Tag,'--repo','0xAgentive/agentic-pipeline','--clobber',$ReleaseZip,$CompanionZip,$RuntimeZip)
