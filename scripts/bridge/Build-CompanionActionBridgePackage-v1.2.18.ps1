...
if([string]::IsNullOrWhiteSpace($OutputRoot)){$OutputRoot=Join-Path $Root '.artifacts\release-kit\1.2.18\action-bridge'}
$Output=[IO.Path]::GetFullPath($OutputRoot)
New-Item -ItemType Directory -Force $Output|Out-Null
$Zip=Join-Path $Output 'agentic-action-bridge-1.2.18.zip'
...
$Stage=Join-Path ([IO.Path]::GetTempPath()) ('action-bridge-1.2.18-'+[guid]::NewGuid().ToString('N'))
$Pack=Join-Path $Stage 'agentic-action-bridge-1.2.18'
...
try{
  ...
  $Version=[ordered]@{schema_version='1.0.0';ecosystem_version='1.2.18';component='action_bridge';version='1.2.18';source_commit=$Commit;primary_packet_extension='.json';external_packet_authorization='local_capability_injection'}
  ...
  $Manifest=[ordered]@{schema_version='1.0.0';ecosystem_version='1.2.18';component='action_bridge';source_commit=$Commit;files=$Files}
