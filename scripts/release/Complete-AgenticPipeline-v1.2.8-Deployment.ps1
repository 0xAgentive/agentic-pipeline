...
if([string]::IsNullOrWhiteSpace($DeploymentRoot)){$DeploymentRoot=Join-Path $env:USERPROFILE (Join-Path 'Documents\antigravity\companion-deployments' (Join-Path $ProjectId '1.2.8'))}
$Version=Get-Content -LiteralPath (Join-Path $Repo 'VERSION.json') -Raw -Encoding UTF8|ConvertFrom-Json
if([string]$Version.package_version-ne'1.2.8'-or[string]$Version.runtime_version-ne'1.2.8'-or[string]$Version.companion_version-ne'1.2.8'){throw 'Canonical repository is not Pipeline 1.2.8 / runtime 1.2.8 / Companion 1.2.8.'}
$RuntimeUpdater=Join-Path $Repo 'scripts\windows\Update-AgenticProjectRuntime-v1.2.8.ps1'
...
$HandoffUpdater=Join-Path $Repo 'integrations\companion-handoff-1.2.8\Update-AgenticContextHandoff-v1.2.8.ps1'
& $HandoffUpdater -HandoffRoot $HandoffRoot -Apply
if($LASTEXITCODE-ne0){throw 'Companion Handoff compatibility update failed.'}
$CompanionZip=Join-Path $Repo '.artifacts\release-kit\1.2.8\companion\agentic-companion-1.2.8.zip'
if(-not(Test-Path -LiteralPath $CompanionZip -PathType Leaf)){& (Join-Path $Repo 'scripts\windows\companion\Build-CompanionPack-v1.2.8.ps1') -RepoRoot $Repo -OutputRoot (Split-Path -Parent $CompanionZip) -Force;if($LASTEXITCODE-ne0){throw 'Companion package build failed.'}}
...
$Instructions = Join-Path $DeploymentRoot '01_PROJECT_INSTRUCTIONS_v1.2.8.md'
...
$BootstrapScript=Join-Path $Repo 'scripts\release\Create-Companion-Restart-Bootstrap-v1.2.8.ps1'
...
$Checklist=@"
Companion 1.2.8 deployment is ready for project: $LogicalName

1. Replace the ChatGPT Project Instructions with 01_PROJECT_INSTRUCTIONS_v1.2.8.md (already copied to clipboard).
...
$Result=[ordered]@{schema_version='1.2.8';status='PASS';project_id=$ProjectId;project=$LogicalName;project_root=$Project;pipeline_version='1.2.8';runtime_version='1.2.8';companion_version='1.2.8';deployment_root=$DeploymentRoot;bootstrap=$Bootstrap.FullName;generated_at_utc=(Get-Date).ToUniversalTime().ToString('o')}
