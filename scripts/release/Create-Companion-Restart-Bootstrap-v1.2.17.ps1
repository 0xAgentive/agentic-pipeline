...
if ([string](Get-MapValue -Map $Deployment -Key 'ecosystem_version') -ne $EcosystemVersion -or
  ...
  throw 'Deployment manifest is not the Companion 1.2.17 derivative deployment.'
}
...
try {
  ...
  $CompanionManifestEntry = $CompanionZipObject.GetEntry('agentic-companion-1.2.17/MANIFEST.json')
  ...
  if ([string](Get-MapValue -Map $PackageActiveSet -Key 'project_instructions') -ne '01_PROJECT_INSTRUCTIONS_v1.2.17.md' -or
  ...
  if ([string](Get-MapValue -Map $DeploymentActiveSet -Key 'project_instructions') -ne '01_PROJECT_INSTRUCTIONS_v1.2.17.md' -or
  ...
  $ExpectedActivePaths = @('01_PROJECT_INSTRUCTIONS_v1.2.17.md') + @($ExpectedModules | ForEach-Object { "knowledge/$_" })
  ...
  foreach ($File in $PackageFiles) {
    ...
    $AssetEntry = $CompanionZipObject.GetEntry("agentic-companion-1.2.17/$Relative")
  ...
  $ExpectedAssetEntryNames = @($ExpectedPayloadPaths | ForEach-Object { "agentic-companion-1.2.17/$_" }) + 'agentic-companion-1.2.17/MANIFEST.json'
...
$OperationRoot = Join-Path ([IO.Path]::GetTempPath()) ("companion-bootstrap-1.2.17-$OperationId")
...
$ZipName = "COMPANION_RESTART_BOOTSTRAP_${ProjectId}_1.2.17.zip"
...
"@.Trim()
  ...
  Remove-SafeTemporaryDirectory -Path $OperationRoot -LeafPrefix 'companion-bootstrap-1.2.17-'
}
