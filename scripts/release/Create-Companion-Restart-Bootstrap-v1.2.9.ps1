...
if([string]::IsNullOrWhiteSpace($OutputRoot)){$OutputRoot=Join-Path $env:USERPROFILE (Join-Path 'Documents\antigravity\companion-deployments' (Join-Path $ProjectId '1.2.9'))}
$Version=Get-Content -LiteralPath (Join-Path $Pipeline 'VERSION.json') -Raw -Encoding UTF8|ConvertFrom-Json
if([string]$Version.package_version-ne'1.2.9'-or[string]$Version.runtime_version-ne'1.2.9'-or[string]$Version.companion_version-ne'1.2.9'){throw 'Restart bootstrap requires Pipeline 1.2.9 / runtime 1.2.9 / Companion 1.2.9.'}
...
Pipeline 1.2.9, runtime 1.2.9 and Companion 1.2.9 are active.
...
"@
 ...
 $StateSummary=[ordered]@{schema_version='1.2.9';project=$LogicalName;project_id=$ProjectId;runtime_root=$Project;source_root=$SourceRoot;pipeline_package='1.2.9';runtime_version='1.2.9';companion_version='1.2.9';current_goal=$CurrentGoal;work_item_status=$WorkItemStatus;next_route=$NextRoute;generated_at_utc=(Get-Date).ToUniversalTime().ToString('o')}
...
"@
 ...
 $Manifest=[ordered]@{schema_version='1.2.9';artifact='COMPANION_RESTART_BOOTSTRAP';project_id=$ProjectId;project=$LogicalName;pipeline_package='1.2.9';runtime_version='1.2.9';companion_version='1.2.9';file_count=$ManifestFiles.Count;files=$ManifestFiles;generated_at_utc=(Get-Date).ToUniversalTime().ToString('o')}
 ...
 $Result=[ordered]@{schema_version='1.2.9';status='PASS';project_id=$ProjectId;project=$LogicalName;zip_path=$ZipPath;zip_sha256=(Get-FileHash -LiteralPath $ZipPath -Algorithm SHA256).Hash.ToLowerInvariant();file_count=$ManifestFiles.Count;source_root=$SourceRoot;generated_at_utc=(Get-Date).ToUniversalTime().ToString('o')}
