...
param(
  [string]$CompanionZip = '',
  [string]$OutputRoot = "$env:USERPROFILE\Downloads\Agentic-Pipeline-Companion-1.2.16",
...
function Test-PackageRoot {
  ...
  if ($Instruction -ne '01_PROJECT_INSTRUCTIONS_v1.2.16.md') { throw 'Unexpected active Project Instructions file.' }
  $ExpectedModules = @(
...
if ([string]::IsNullOrWhiteSpace($CompanionZip)) {
  $KitCandidate = Join-Path $PSScriptRoot 'prebuilt\agentic-companion-1.2.16.zip'
  $RepoCandidate = Join-Path $CanonicalRoot '.artifacts\release-kit\1.2.16\companion\agentic-companion-1.2.16.zip'
...
if ([string]::IsNullOrWhiteSpace($CompanionZip) -or -not (Test-Path -LiteralPath $CompanionZip -PathType Leaf)) {
  throw 'Companion 1.2.16 ZIP was not found. Pass -CompanionZip explicitly.'
}
...
$ExtractRoot = Join-Path ([IO.Path]::GetTempPath()) ("companion-prepare-1.2.16-$OperationId")
$MovedAside = $false
$Installed = $false
...
try {
  ...
  try {
    ...
    foreach ($Name in $Names) {
      ...
      if (-not $Name.StartsWith('agentic-companion-1.2.16/', [StringComparison]::Ordinal)) {
  ...
  $PackageRoot = Join-Path $ExtractRoot 'agentic-companion-1.2.16'
  ...
  if ((Test-Path -LiteralPath $OutputFull -PathType Container) -and
    ...
    $InstructionsPath = Join-Path $OutputFull '01_PROJECT_INSTRUCTIONS_v1.2.16.md'
  ...
  $Installed = $true
  $Instructions = Join-Path $OutputFull '01_PROJECT_INSTRUCTIONS_v1.2.16.md'
  if (-not $SkipClipboard) { Set-Clipboard -Value (Get-Content -LiteralPath $Instructions -Raw -Encoding UTF8) }
  Write-Host 'COMPANION 1.2.16 DEPLOYMENT PREPARED AND VERIFIED.' -ForegroundColor Green
  ...
  Remove-SafeTemporaryDirectory -Path $ExtractRoot -LeafPrefix 'companion-prepare-1.2.16-'
}
