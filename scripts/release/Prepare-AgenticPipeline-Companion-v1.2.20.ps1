...
param(
  [string]$CompanionZip = '',
  [string]$OutputRoot = "$env:USERPROFILE\Downloads\Agentic-Pipeline-Companion-1.2.20",
...
function Test-PackageRoot {
  ...
  if ($Instruction -ne '01_PROJECT_INSTRUCTIONS_v1.2.20.md') { throw 'Unexpected active Project Instructions file.' }
  $ExpectedModules = @(
    '00_AGENTIC_PIPELINE_INDEX_v1.2.20.md', '01_CONTEXT_SPLIT_POLICY.md', '02_AGENT_TASK_PACK_CONTRACT_v1.2.20.md',
...
if ([string]::IsNullOrWhiteSpace($CompanionZip)) {
  $KitCandidate = Join-Path $PSScriptRoot 'prebuilt\agentic-companion-1.2.20.zip'
  $RepoCandidate = Join-Path $CanonicalRoot '.artifacts\release-kit\1.2.20\companion\agentic-companion-1.2.20.zip'
...
if ([string]::IsNullOrWhiteSpace($CompanionZip) -or -not (Test-Path -LiteralPath $CompanionZip -PathType Leaf)) {
  throw 'Companion 1.2.20 ZIP was not found. Pass -CompanionZip explicitly.'
}
...
$ExtractRoot = Join-Path ([IO.Path]::GetTempPath()) ("companion-prepare-1.2.20-$OperationId")
$MovedAside = $false
$Installed = $false
...
try {
  ...
  try {
    ...
    foreach ($Name in $Names) {
      ...
      if (-not $Name.StartsWith('agentic-companion-1.2.20/', [StringComparison]::Ordinal)) {
  ...
  $PackageRoot = Join-Path $ExtractRoot 'agentic-companion-1.2.20'
  ...
  if ((Test-Path -LiteralPath $OutputFull -PathType Container) -and
    ...
    $InstructionsPath = Join-Path $OutputFull '01_PROJECT_INSTRUCTIONS_v1.2.20.md'
  ...
  $Installed = $true
  $Instructions = Join-Path $OutputFull '01_PROJECT_INSTRUCTIONS_v1.2.20.md'
  if (-not $SkipClipboard) { Set-Clipboard -Value (Get-Content -LiteralPath $Instructions -Raw -Encoding UTF8) }
  Write-Host 'COMPANION 1.2.20 DEPLOYMENT PREPARED AND VERIFIED.' -ForegroundColor Green
  ...
  Remove-SafeTemporaryDirectory -Path $ExtractRoot -LeafPrefix 'companion-prepare-1.2.20-'
}
