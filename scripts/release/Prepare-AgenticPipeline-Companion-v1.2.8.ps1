...
param(
  [string]$CompanionZip = "",
  [string]$OutputRoot = "$env:USERPROFILE\Downloads\Agentic-Pipeline-Companion-1.2.8",
  [switch]$OpenFolder,
...
if ([string]::IsNullOrWhiteSpace($CompanionZip)) {
  $KitCandidate = Join-Path $PSScriptRoot 'prebuilt\agentic-companion-1.2.8.zip'
  if (Test-Path -LiteralPath $KitCandidate -PathType Leaf) {
    $CompanionZip = $KitCandidate
  }
  else {
    $RepoCandidate = Join-Path (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)) '.artifacts\release-kit\1.2.8\companion\agentic-companion-1.2.8.zip'
...
if ([string]::IsNullOrWhiteSpace($CompanionZip) -or !(Test-Path -LiteralPath $CompanionZip -PathType Leaf)) {
  throw 'Companion 1.2.8 ZIP was not found. Pass -CompanionZip explicitly.'
}
...
$Instructions = Get-ChildItem -LiteralPath $OutputRoot -Recurse -File -Filter '01_PROJECT_INSTRUCTIONS_v1.2.8.md' | Select-Object -First 1
...
Write-Host 'COMPANION 1.2.8 PACKAGE PREPARED.' -ForegroundColor Green
