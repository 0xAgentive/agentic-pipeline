...
if ([string]($Version.package_version) -ne '1.2.8') { throw 'Local repository is not version 1.2.8.' }
...
if ($TagLines.Count -gt 0) {
  ...
  if ($LocalTag) {
    ...
    Invoke-Native -FilePath git -ArgumentList @('-C',$Repo,'tag','-a',$Tag,'-m','Agentic Pipeline 1.2.8 - Owner-Autonomous Execution') | Out-Null
  }
...
$ReleaseZip = Join-Path $ArtifactRoot 'pipeline\agentic-pipeline-1.2.8.zip'
$CompanionZip = Join-Path $ArtifactRoot 'companion\agentic-companion-1.2.8.zip'
$RuntimeZip = Join-Path $ArtifactRoot 'runtime\agentic-project-runtime-1.2.8.zip'
$ActionBridgeZip = Join-Path $ArtifactRoot 'action-bridge\agentic-action-bridge-1.2.8.zip'
$HandoffCompatibilityZip = Join-Path $ArtifactRoot 'handoff-compatibility\agentic-context-handoff-1.2.8.zip'
...
$Notes = Join-Path $ArtifactRoot 'RELEASE_NOTES_1.2.8.md'
@'
# Agentic Pipeline 1.2.8 - Owner-Autonomous Execution
...
- Companion 1.2.8 with compact finding/repair delta output;
- standalone Antigravity runtime 1.2.8 overlay;
- active pre-write and command hooks;
- Action Bridge 1.2.8 for downloaded Companion packets;
- Auto Context Handoff compatibility 1.2.8;
...
if ($Existing.Code -ne 0) {
  Invoke-Native -FilePath $Gh -ArgumentList @(
    ...
    '--title','Agentic Pipeline 1.2.8 - Owner-Autonomous Execution',
    '--notes-file',$Notes,
