...
$LocalScript = Join-Path $PSScriptRoot 'Run-AgenticPipeline-v1.2.13-Upgrade-Local.ps1'
...
try {
  $PublishScript = Join-Path $PSScriptRoot 'Publish-AgenticPipeline-v1.2.13-GitHub.ps1'
  ...
  Write-Warning 'The local repository and release artifacts remain available. Re-run Publish-AgenticPipeline-v1.2.13-GitHub.ps1 after network recovery.'
  throw
}
...
if(-not$SkipProjectDeployment){
  ...
  $ArtifactRoot=Join-Path $RepoRoot '.artifacts\release-kit\1.2.13'
  if([string]::IsNullOrWhiteSpace($RuntimeAsset)){$RuntimeAsset=Join-Path $ArtifactRoot 'runtime\agentic-project-runtime-1.2.13.zip'}
  if([string]::IsNullOrWhiteSpace($ActionBridgeAsset)){$ActionBridgeAsset=Join-Path $ArtifactRoot 'action-bridge\agentic-action-bridge-1.2.13.zip'}
  if([string]::IsNullOrWhiteSpace($ContextHandoffAsset)){$ContextHandoffAsset=Join-Path $ArtifactRoot 'handoff-compatibility\agentic-context-handoff-1.2.13.zip'}
  if([string]::IsNullOrWhiteSpace($CompanionAsset)){$CompanionAsset=Join-Path $ArtifactRoot 'companion\agentic-companion-1.2.13.zip'}
  ...
  $DeployScript=Join-Path $RepoRoot 'scripts\release\Complete-AgenticPipeline-v1.2.13-Deployment.ps1'
...
Write-Host 'AGENTIC PIPELINE 1.2.13 GLOBAL FIX COMPLETED.' -ForegroundColor Green
