...
$KitVerifier = Join-Path $KitRoot 'Verify-AgenticPipeline-v1.2.13-Kit.ps1'
...
if ([string]($BeforeVersion.package_version) -notin @('1.2.6','1.2.9','1.2.10','1.2.11','1.2.12','1.2.13')) { throw "Unsupported baseline package: $($BeforeVersion.package_version)" }

# Validate the exact overlay in an isolated detached worktree before touching local main.
if ([string]($BeforeVersion.package_version) -in @('1.2.6', '1.2.9', '1.2.10', '1.2.11', '1.2.12') -or $ForceReapply) {
  $CandidateRoot = Join-Path $env:TEMP ('agentic-pipeline-1.2.13-candidate-' + [guid]::NewGuid().ToString('N'))
  ...
  try {
    ...
    if ($CandidateChanges.Count -lt 1) { throw 'The isolated 1.2.13 candidate contains no overlay changes.' }
    Invoke-Native -FilePath git -ArgumentList @(
      ...
      'commit','-m','validation: isolated Agentic Pipeline 1.2.13 candidate'
    ) | Out-Null

    $CandidateArtifactRoot = Join-Path $env:TEMP ('agentic-pipeline-1.2.13-release-preflight-' + [guid]::NewGuid().ToString('N'))
    try {
      Invoke-RepositoryScript -RepositoryRoot $CandidateRoot -RelativePath 'scripts\windows\Build-ReleasePackage.ps1' -ArgumentList @('-RepoRoot',$CandidateRoot,'-Version','1.2.13','-OutputRoot',(Join-Path $CandidateArtifactRoot 'pipeline'),'-Force') -DisplayName 'isolated build Pipeline release package' | Out-Null
      Invoke-RepositoryScript -RepositoryRoot $CandidateRoot -RelativePath 'scripts\windows\companion\Build-CompanionPack-v1.2.13.ps1' -ArgumentList @('-RepoRoot',$CandidateRoot,'-OutputRoot',(Join-Path $CandidateArtifactRoot 'companion'),'-Force') -DisplayName 'isolated build Companion pack' | Out-Null
      Invoke-RepositoryScript -RepositoryRoot $CandidateRoot -RelativePath 'scripts\windows\Build-AgenticProjectRuntimeOverlay-v1.2.13.ps1' -ArgumentList @('-RepoRoot',$CandidateRoot,'-OutputRoot',(Join-Path $CandidateArtifactRoot 'runtime'),'-Force') -DisplayName 'isolated build Antigravity runtime overlay' | Out-Null
      Invoke-RepositoryScript -RepositoryRoot $CandidateRoot -RelativePath 'scripts\bridge\Build-CompanionActionBridgePackage-v1.2.13.ps1' -ArgumentList @('-RepoRoot',$CandidateRoot,'-OutputRoot',(Join-Path $CandidateArtifactRoot 'action-bridge'),'-Force') -DisplayName 'isolated build Action Bridge package' | Out-Null
      Invoke-RepositoryScript -RepositoryRoot $CandidateRoot -RelativePath 'integrations\companion-handoff-1.2.13\Build-AgenticContextHandoffPackage-v1.2.13.ps1' -ArgumentList @('-RepoRoot',$CandidateRoot,'-OutputRoot',(Join-Path $CandidateArtifactRoot 'handoff-compatibility'),'-Force') -DisplayName 'isolated build Context Handoff package' | Out-Null

      Test-ZipArtifactSafety -ArchivePath (Join-Path $CandidateArtifactRoot 'pipeline\agentic-pipeline-1.2.13.zip') -Label 'Isolated Pipeline release ZIP'
      Test-ZipArtifactSafety -ArchivePath (Join-Path $CandidateArtifactRoot 'companion\agentic-companion-1.2.13.zip') -Label 'Isolated Companion ZIP'
      Test-ZipArtifactSafety -ArchivePath (Join-Path $CandidateArtifactRoot 'runtime\agentic-project-runtime-1.2.13.zip') -Label 'Isolated runtime overlay ZIP'
      Test-ZipArtifactSafety -ArchivePath (Join-Path $CandidateArtifactRoot 'action-bridge\agentic-action-bridge-1.2.13.zip') -Label 'Isolated Action Bridge ZIP'
      Test-ZipArtifactSafety -ArchivePath (Join-Path $CandidateArtifactRoot 'handoff-compatibility\agentic-context-handoff-1.2.13.zip') -Label 'Isolated Handoff compatibility ZIP'
    ...
    Write-Host 'ISOLATED 1.2.13 CANDIDATE VALIDATION PASSED.' -ForegroundColor Green
  }
  finally {
    if ($CandidateAdded) {
...
$BackupBranch=$null
$ArtifactRoot=Join-Path $Repo '.artifacts\release-kit\1.2.13'
try {
  if ([string]($BeforeVersion.package_version) -eq '1.2.13' -and !$ForceReapply) {
    Write-Host 'Local repository already declares 1.2.13. Running validation and package build only.'
  }
  else {
    $BackupBranch="backup/pre-v1.2.13-$Stamp"
  ...
  if ($Changes.Count -gt 0) {
    ...
    Invoke-Native -FilePath git -ArgumentList @('-C',$Repo,'commit','-m','release: Agentic Pipeline 1.2.13 source-immutable finalization') | Out-Null
  }

  $Version=Get-Content -LiteralPath $VersionPath -Raw | ConvertFrom-Json
  if ([string]($Version.package_version) -ne '1.2.13' -or [string]($Version.runtime_version) -ne '1.2.13' -or [string]($Version.companion_version) -ne '1.2.13') {
    throw 'Version matrix is not 1.2.13 / 1.2.13 / 1.2.13 after update.'
  }
  ...
  Invoke-RepositoryScript -RepositoryRoot $Repo -RelativePath 'scripts\windows\Build-ReleasePackage.ps1' -ArgumentList @('-RepoRoot',$Repo,'-Version','1.2.13','-OutputRoot',(Join-Path $ArtifactRoot 'pipeline'),'-Force') -DisplayName 'build Pipeline release package' | Out-Null
  Invoke-RepositoryScript -RepositoryRoot $Repo -RelativePath 'scripts\windows\companion\Build-CompanionPack-v1.2.13.ps1' -ArgumentList @('-RepoRoot',$Repo,'-OutputRoot',(Join-Path $ArtifactRoot 'companion'),'-Force') -DisplayName 'build Companion pack' | Out-Null
  Invoke-RepositoryScript -RepositoryRoot $Repo -RelativePath 'scripts\windows\Build-AgenticProjectRuntimeOverlay-v1.2.13.ps1' -ArgumentList @('-RepoRoot',$Repo,'-OutputRoot',(Join-Path $ArtifactRoot 'runtime'),'-Force') -DisplayName 'build Antigravity runtime overlay' | Out-Null
  Invoke-RepositoryScript -RepositoryRoot $Repo -RelativePath 'scripts\bridge\Build-CompanionActionBridgePackage-v1.2.13.ps1' -ArgumentList @('-RepoRoot',$Repo,'-OutputRoot',(Join-Path $ArtifactRoot 'action-bridge'),'-Force') -DisplayName 'build Action Bridge package' | Out-Null
  Invoke-RepositoryScript -RepositoryRoot $Repo -RelativePath 'integrations\companion-handoff-1.2.13\Build-AgenticContextHandoffPackage-v1.2.13.ps1' -ArgumentList @('-RepoRoot',$Repo,'-OutputRoot',(Join-Path $ArtifactRoot 'handoff-compatibility'),'-Force') -DisplayName 'build Context Handoff package' | Out-Null

  $Result=[ordered]@{
    schema_version='1.2.9';status='PASS';operation='local_upgrade';repo_root=$Repo;origin=$Origin;branch='main';head=$Head
    initial_head=$InitialHead;package_version='1.2.13';runtime_version='1.2.13';companion_version='1.2.13';artifact_root=$ArtifactRoot
    network_transport_profiles=@($NetworkProfiles | Select-Object -Unique)
    release_zip=(Join-Path $ArtifactRoot 'pipeline\agentic-pipeline-1.2.13.zip')
    companion_zip=(Join-Path $ArtifactRoot 'companion\agentic-companion-1.2.13.zip')
    runtime_overlay_zip=(Join-Path $ArtifactRoot 'runtime\agentic-project-runtime-1.2.13.zip')
    action_bridge_zip=(Join-Path $ArtifactRoot 'action-bridge\agentic-action-bridge-1.2.13.zip')
    handoff_compatibility_zip=(Join-Path $ArtifactRoot 'handoff-compatibility\agentic-context-handoff-1.2.13.zip')
    completed_at_utc=(Get-Date).ToUniversalTime().ToString('o')
  }
  $ResultPath=Join-Path $ArtifactRoot 'LOCAL_UPGRADE_RESULT.json'
  Write-Json $ResultPath $Result

  Write-Host 'LOCAL AGENTIC PIPELINE 1.2.13 UPGRADE COMPLETED.' -ForegroundColor Green
