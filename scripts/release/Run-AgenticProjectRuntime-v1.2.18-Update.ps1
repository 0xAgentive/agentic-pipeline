...
try {
  if ([string]::IsNullOrWhiteSpace($RuntimeZip)) {
    $KitCandidate = Join-Path $PSScriptRoot 'prebuilt\agentic-project-runtime-1.2.18.zip'
  ...
  if (![string]::IsNullOrWhiteSpace($RuntimeZip)) {
    ...
    $TempRoot = Join-Path ([IO.Path]::GetTempPath()) ('agentic-project-runtime-1.2.18-' + [guid]::NewGuid().ToString('N'))
    ...
    $VersionFile = Get-ChildItem -LiteralPath $TempRoot -Recurse -File -Filter 'VERSION.json' | Where-Object {
      try {
        ...
        [string]($V.package_version) -eq '1.2.18' -and [string]($V.runtime_version) -eq '1.2.18'
      } catch { $false }
  ...
  $Updater = Join-Path $EffectiveRepoRoot 'scripts\windows\Update-AgenticProjectRuntime-v1.2.18.ps1'
  ...
  if ($Apply) {
    Write-Host 'PROJECT RUNTIME 1.2.18 UPDATE COMPLETED.' -ForegroundColor Green
  }
  else {
    Write-Host 'PROJECT RUNTIME 1.2.18 DRY RUN COMPLETED. Re-run with -Apply after review.' -ForegroundColor Yellow
  }
}
finally {
