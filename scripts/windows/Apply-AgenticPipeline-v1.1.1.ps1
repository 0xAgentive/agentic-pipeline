...
$PlaybookSrc = Join-Path (Split-Path (Split-Path $ScriptDir -Parent) -Parent) "docs\AGENTIC_PIPELINE_PLAYBOOK.md"; if (!(Test-Path $PlaybookSrc)) { $PlaybookSrc = Join-Path (Split-Path (Split-Path $ScriptDir -Parent) -Parent) "docs\AGENTIC_PIPELINE_PLAYBOOK.md"
if (!(Test-Path $PlaybookSrc)) {
  throw "Canonical playbook missing: $PlaybookSrc"
} }
