...
if ([string]$Version.ecosystem_version -ne '1.2.15' -or [string]$Version.package_version -ne '1.2.15' -or [string]$Version.runtime_version -ne '1.2.15') {
  throw "Runtime overlay requires unified ecosystem 1.2.15."
}
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
  $OutputRoot = Join-Path $Root '.artifacts\runtime\1.2.15'
}
New-Item -ItemType Directory -Force -Path $OutputRoot | Out-Null
$ZipPath = Join-Path $OutputRoot 'agentic-project-runtime-1.2.15.zip'
...
$Files = @(
  ...
  'scripts\windows\Update-AgenticProjectRuntime-v1.2.15.ps1',
  ...
  'scripts\windows\Test-KnownFailureRegressionPlaybook-v1.2.15.ps1',
...
$PackageRoot = Join-Path $Temp 'agentic-project-runtime-1.2.15'
