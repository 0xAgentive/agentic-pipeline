...
if ([string]$Version.ecosystem_version -ne '1.2.19' -or [string]$Version.package_version -ne '1.2.19' -or [string]$Version.runtime_version -ne '1.2.19') {
  throw "Runtime overlay requires unified ecosystem 1.2.19."
}
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
  $OutputRoot = Join-Path $Root '.artifacts\runtime\1.2.19'
}
New-Item -ItemType Directory -Force -Path $OutputRoot | Out-Null
$ZipPath = Join-Path $OutputRoot 'agentic-project-runtime-1.2.19.zip'
...
$Files = @(
  ...
  'scripts\windows\Update-AgenticProjectRuntime-v1.2.19.ps1',
  ...
  'scripts\windows\Test-KnownFailureRegressionPlaybook-v1.2.19.ps1',
  'tests\regression\known-failure-regression.cjs',
  'tests\regression\KNOWN_FAILURE_PLAYBOOK_v1.2.19.json',
...
$PackageRoot = Join-Path $Temp 'agentic-project-runtime-1.2.19'
