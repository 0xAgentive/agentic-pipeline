...
try {
  ...
  $Updater = Join-Path $Root 'scripts\windows\Update-AgenticProjectRuntime-v1.2.23.ps1'
  Invoke-Checked -FilePath $Pwsh -ArgumentList @(
  ...
  if ([string]$InstallManifest.runtime_version -ne '1.2.23') { throw 'Runtime version mismatch after update.' }
  ...
  [System.IO.File]::WriteAllText((Join-Path $Project '.agy\ACTION_BRIDGE_CAPABILITY.json'), ([ordered]@{schema_version='1.2.9';ecosystem_version='1.2.23';project_id='operational-test';capability_token=$CapabilityToken}|ConvertTo-Json), $Utf8NoBom)
