...
try {
  ...
  if ([string]$InstallManifest.runtime_version -ne '1.2.10') { throw 'Runtime version mismatch after update.' }
  ...
  [System.IO.File]::WriteAllText((Join-Path $Project '.agy\ACTION_BRIDGE_CAPABILITY.json'), ([ordered]@{schema_version='1.2.9';ecosystem_version='1.2.10';project_id='operational-test';capability_token=$CapabilityToken}|ConvertTo-Json), $Utf8NoBom)
