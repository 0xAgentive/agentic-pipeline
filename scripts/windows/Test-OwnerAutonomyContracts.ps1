...
const c=require('./scripts/control-plane/autonomous-convergence.cjs');const p=require('./scripts/control-plane/progress-guard.cjs');const f=require('./scripts/control-plane/validate-findings.cjs');const s=require('./scripts/control-plane/validate-owner-summary.cjs');const a=require('./scripts/control-plane/action-packet.cjs');
...
const packet={schema_version:'1.2.8',ecosystem_version:'1.2.8',packet_format:'single_json',packet_id:'packet-contract-001',project_id:'H10-Athlete-Cardio-Lab',capability_token:'a'.repeat(64),operation:'continue_work_item',route:'/fixcritical',work_item_id:'wi-1',goal_epoch:3,goal:'Continue current goal',assurance_mode:'guarded',owner_approved:true,owner_interaction_policy:'hard_stop_only',scope_binding:'executor_discovery',technical_task_markdown:'# Task\nContinue current work.',owner_summary_ru:plain,created_at_utc:new Date(Date.now()-1000).toISOString(),expires_at_utc:new Date(Date.now()+3600000).toISOString()};if(!a.validatePacketObject(packet).ok)throw new Error('valid JSON packet rejected');packet.expires_at_utc=new Date(Date.now()-100).toISOString();if(a.validatePacketObject(packet).ok)throw new Error('expired packet accepted');console.log(JSON.stringify({ok:true,checks:6}));
'@
$Temp=Join-Path $Root '.owner-autonomy-contract-test.cjs'
try{[IO.File]::WriteAllText($Temp,$TestScript,[Text.UTF8Encoding]::new($false));& $Node $Temp;if($LASTEXITCODE-ne0){throw 'Owner-autonomy Node contracts failed.'}}finally{Remove-Item $Temp -Force -ErrorAction SilentlyContinue}
$PythonCommand=Get-Command python -ErrorAction SilentlyContinue|Select-Object -First 1
if(-not$PythonCommand){$PythonCommand=Get-Command py -ErrorAction Stop|Select-Object -First 1}
& $PythonCommand.Source -m py_compile (Join-Path $Root 'scripts\bridge\companion_action_bridge.py')
if($LASTEXITCODE-ne0){throw 'Action Bridge Python compile failed.'}
