...
$Branch=(@(& git -C $Root branch --show-current 2>&1)-join"`n").Trim();if($LASTEXITCODE-ne0-or[string]::IsNullOrWhiteSpace($Branch)){throw 'Git branch unavailable.'}
$Head=(@(& git -C $Root rev-parse HEAD 2>&1)-join"`n").Trim();if($LASTEXITCODE-ne0){throw 'Git HEAD unavailable.'}
$GitRoot=(@(& git -C $Root rev-parse --show-toplevel 2>&1)-join"`n").Trim();if($LASTEXITCODE-ne0){throw 'Git root unavailable.'}
...
$GitStatus=@(& git -C $Root status --porcelain=v1 --untracked-files=all 2>&1);if($LASTEXITCODE-ne0){throw 'Git status unavailable.'}
...
$Normalized=@($AllowedPaths|ForEach-Object{([string]$_ -replace'\\','/').Trim().TrimStart('./')}|Where-Object{$_}|Sort-Object -Unique)
...
$Firewall.algorithm_repair_authorized=[bool]$AuthorizeAlgorithmRepair
$Firewall.algorithm_repair_finding_ids=@($FindingIds|Sort-Object -Unique)
$Firewall.analytical_baseline_head=$Head
$Firewall.generated_at_utc=$Now
$Handshake=[ordered]@{schema_version='1.2.0';generated_at_utc=$Now;project_root=$Root;git=[ordered]@{branch=$Branch;head_commit=$Head;git_state=if($GitStatus.Count){'dirty'}else{'clean'}};installed=[ordered]@{package_version='1.2.8';runtime_version='1.2.8';companion_version='1.2.8'};work_item=[ordered]@{work_item_id=[string]$Wi.work_item_id;goal_epoch=[int]$Wi.goal_epoch;status='active'};routing=[ordered]@{routing_valid=$true;resolved_commands_allowed_now=@($Route);next_required_command=$Route;product_execution_allowed=($Route-in@('/nextphase','/fixcritical','/fastpatch'));release_execution_allowed=($Route-eq'/shipcheck')};execution_lease_id=$Lease.lease_id;progress_guard=[ordered]@{numeric_repair_budget=$false;auto_continue_while_progress=$true;consecutive_no_progress_limit=2;same_failure_limit=2}}
