...
function Test-PackageTree {
  ...
  $ExpectedInstruction = '01_PROJECT_INSTRUCTIONS_v1.2.14.md'
  ...
  $ExpectedModules = @(
    '00_AGENTIC_PIPELINE_INDEX_v1.2.14.md',
    '01_CONTEXT_SPLIT_POLICY.md',
    '02_AGENT_TASK_PACK_CONTRACT_v1.2.14.md',
...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
  $OutputRoot = Join-Path $Root '.artifacts\release-kit\1.2.14\companion'
}
...
$BuildStage = Join-Path ([IO.Path]::GetTempPath()) ("companion-1.2.14-$OperationId")
$CheckStage = Join-Path ([IO.Path]::GetTempPath()) ("companion-check-1.2.14-$OperationId")
$PackageRoot = Join-Path $BuildStage 'agentic-companion-1.2.14'
...
try {
  ...
  $InstructionName = '01_PROJECT_INSTRUCTIONS_v1.2.14.md'
...
"@.Trim()
  ...
  $ZipPath = Join-Path $OutputStage 'agentic-companion-1.2.14.zip'
  ...
  [void](Test-PackageTree -PackageRoot (Join-Path $CheckStage 'agentic-companion-1.2.14'))
  $AssetHash = Get-Sha256 -Path $ZipPath
  Write-Utf8File -Path (Join-Path $OutputStage 'agentic-companion-1.2.14.zip.sha256') -Text "$AssetHash  agentic-companion-1.2.14.zip`n"
  $BuildResult = [ordered]@{
    schema_version = '1.0.0'
    status = 'PASS'
    ecosystem_version = $EcosystemVersion
    ...
    asset = [ordered]@{ file = 'agentic-companion-1.2.14.zip'; sha256 = $AssetHash; size_bytes = (Get-Item -LiteralPath $ZipPath).Length }
  ...
  Write-Host "Companion pack built and verified: $(Join-Path $OutputFull 'agentic-companion-1.2.14.zip')"
}
catch {
  ...
  Remove-SafeTemporaryDirectory -Path $BuildStage -LeafPrefix 'companion-1.2.14-'
  Remove-SafeTemporaryDirectory -Path $CheckStage -LeafPrefix 'companion-check-1.2.14-'
}
