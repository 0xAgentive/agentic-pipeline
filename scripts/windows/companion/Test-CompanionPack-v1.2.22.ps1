...
try{
 ...
 if([string]$Version.companion_version-ne'1.2.22'-or[string]$Version.ecosystem_version-ne'1.2.22'){Add-Failure "Unexpected Companion version: $($Version.companion_version)"}
}catch{Add-Failure "Invalid Companion VERSION.json: $($_.Exception.Message)"}
foreach($Required in @(
 'docs\companion\01_PROJECT_INSTRUCTIONS_v1.2.22.md','docs\companion\00_AGENTIC_PIPELINE_INDEX_v1.2.22.md','docs\companion\02_AGENT_TASK_PACK_CONTRACT_v1.2.22.md',
...
)){if(!(Test-Path -LiteralPath (Join-Path $Root $Required)-PathType Leaf)){Add-Failure "Missing Companion 1.2.22 file: $Required"}}
$TextPath=Join-Path $Root 'docs\companion\01_PROJECT_INSTRUCTIONS_v1.2.22.md'
...
$TaskPath=Join-Path $Root 'docs\companion\02_AGENT_TASK_PACK_CONTRACT_v1.2.22.md'
$Task=if(Test-Path -LiteralPath $TaskPath){Get-Content -LiteralPath $TaskPath -Raw -Encoding UTF8}else{''}
foreach($Marker in @('Agent Task Pack Contract v1.2.22','Executor discovery','Completion','owner_interaction_policy')){if($Task-notmatch[regex]::Escape($Marker)){Add-Failure "Task contract marker missing: $Marker"}}
...
Write-Host 'Companion pack 1.2.22 validation passed.'
exit 0
