...
```mermaid
flowchart TD
    subgraph S1["1. Старт и Архитектура"]
        A["💡 Идея или Существующий код"] --> B["⚡ /new-project или /adopt-project"]
        B --> C["📦 /companion-pack (Экспорт для ИИ)"]
    end

    subgraph S2["2. Проектирование с Компаньоном"]
        C --> D["🧠 ChatGPT / GPT-5 (Стратег)"]
        D --> E["📥 Скачивание Action Packet"]
        E --> F["🌉 Action Bridge (Автозахват за 250мс)"]
    end

    subgraph S3["3. Автономное исполнение"]
        F --> G["🚀 /nextphase (или «делай» /goal)"]
        G --> H["🛠️ Автономный кодинг & Тесты"]
        H --> I["🔍 /auditphase (Независимый аудит)"]
        I -- "Есть дефекты" --> J["🔧 /fixcritical (Изолированный фикс)"]
        J --> I
    end

    subgraph S4["4. Финализация и Релиз"]
        I -- "100% Green" --> K["🚀 /shipcheck & Релиз"]
        K --> L["🔄 Возврат отчёта Компаньону"]
    end

    style S1 fill:#1e293b,stroke:#38bdf8,stroke-width:2px,color:#fff
    style S2 fill:#0f172a,stroke:#818cf8,stroke-width:2px,color:#fff
    style S3 fill:#1e1b4b,stroke:#a855f7,stroke-width:2px,color:#fff
