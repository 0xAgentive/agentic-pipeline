...
$PackageArchivePath = Join-Path $TempRoot 'agentic-action-bridge-1.2.25.zip'
...
try {
  ...
  Write-JsonFile -Path (Join-Path $PackageRoot 'VERSION.json') -Value ([ordered]@{
    schema_version = '1.0.0'; ecosystem_version = '1.2.25'; component = 'action_bridge'; version = '1.2.25'; source_commit = $Commit
  })
  ...
  Write-JsonFile -Path (Join-Path $PackageRoot 'MANIFEST.json') -Value ([ordered]@{
    schema_version = '1.0.0'; ecosystem_version = '1.2.25'; component = 'action_bridge'; source_commit = $Commit; files = @($PackageFiles)
  })
