...
function New-ProjectFixture {
  ...
  Write-Json -Path (Join-Path $Agy 'INSTALLATION_MANIFEST.json') -Value ([ordered]@{
      schema_version = '1.0.0'; package_version = '1.2.24'; runtime_version = '1.2.24'; companion_version = '1.2.24'
    })
...
try {
  ...
  $Build = Invoke-Capture -FilePath $Pwsh -Arguments @(
    '-NoLogo', '-NoProfile', '-File', (Join-Path $Pipeline 'scripts\windows\companion\Build-CompanionPack-v1.2.24.ps1'),
  ...
  $CompanionAsset = Join-Path $AssetRoot 'agentic-companion-1.2.24.zip'
  ...
  $Prepare = Invoke-Capture -FilePath $Pwsh -Arguments @(
    '-NoLogo', '-NoProfile', '-File', (Join-Path $Pipeline 'scripts\release\Prepare-AgenticPipeline-Companion-v1.2.24.ps1'),
  ...
  $MixedZip = Join-Path $MixedDeployment 'COMPANION_RESTART_BOOTSTRAP_mixed_1.2.24.zip'
  ...
  $NoEventsZip = Join-Path $NoEventsDeployment 'COMPANION_RESTART_BOOTSTRAP_no-events_1.2.24.zip'
