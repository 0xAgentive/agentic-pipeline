...
$CanonicalUpdater = Join-Path $Root 'integrations\companion-handoff-1.2.11\Update-AgenticContextHandoff-v1.2.11.ps1'
$CompleteScript = Join-Path $Root 'scripts\release\Complete-AgenticPipeline-v1.2.11-Deployment.ps1'
...
$Commit = 'a' * 40
$PackageName = 'agentic-context-handoff-1.2.11'
...
function Invoke-Updater {
  ...
  $Arguments = @(
    '-NoProfile','-ExecutionPolicy','Bypass','-File',(Join-Path $PackageRoot 'Update-AgenticContextHandoff-v1.2.11.ps1'),
...
function New-ContextPackageFixture {
  ...
  Copy-Item -LiteralPath $CanonicalUpdater -Destination (Join-Path $PackageRoot 'Update-AgenticContextHandoff-v1.2.11.ps1')
  ...
  Write-JsonFile -Path (Join-Path $PackageRoot 'VERSION.json') -Value ([ordered]@{
    schema_version = '1.0.0'; ecosystem_version = '1.2.11'; component = 'context_handoff'; version = '1.2.11'; engine_schema_version = '4.3.4'; source_commit = $Commit; source_tree_dirty = $false; source_payload_sha256 = $SourceDigest
  })
  Write-JsonFile -Path (Join-Path $PackageRoot 'SOURCE_ATTESTATION.json') -Value ([ordered]@{
    schema_version = '1.0.0'; ecosystem_version = '1.2.11'; component = 'context_handoff_source'; engine_schema_version = '4.3.4'; source_commit = $Commit; source_tree_dirty = $false; source_payload_sha256 = $SourceDigest; explicit_exclusions = $ExplicitExclusions; source_files = $SourceRecords
  })
  ...
  Write-JsonFile -Path (Join-Path $PackageRoot 'MANIFEST.json') -Value ([ordered]@{
    schema_version = '1.0.0'; ecosystem_version = '1.2.11'; component = 'context_handoff'; version = '1.2.11'; engine_schema_version = '4.3.4'; source_commit = $Commit; source_tree_dirty = $false; built_at_utc = '2026-08-08T00:00:00Z'; source_payload_sha256 = $SourceDigest; package_payload_sha256 = Get-RecordSetDigest -Records $ManifestRecords; explicit_exclusions = $ExplicitExclusions; files = $ManifestRecords
  })
  return $PackageRoot
}
...
try {
  ...
  $ArchivePath = Join-Path $TempRoot 'agentic-context-handoff-1.2.11.zip'
