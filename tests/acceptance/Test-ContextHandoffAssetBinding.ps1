...
$CanonicalUpdater = Join-Path $Root 'integrations/companion-handoff-1.2.26/Update-AgenticContextHandoff-v1.2.26.ps1'
$CompleteScript = Join-Path $Root 'scripts/release/Complete-AgenticPipeline-v1.2.26-Deployment.ps1'
...
$Commit = 'a' * 40
$PackageName = 'agentic-context-handoff-1.2.26'
...
function Invoke-Updater {
  ...
  $Arguments = @(
    '-NoProfile','-ExecutionPolicy','Bypass','-File',(Join-Path $PackageRoot 'Update-AgenticContextHandoff-v1.2.26.ps1'),
...
function New-ContextPackageFixture {
  ...
  Copy-Item -LiteralPath $CanonicalUpdater -Destination (Join-Path $PackageRoot 'Update-AgenticContextHandoff-v1.2.26.ps1')
...
'@
  ...
  Write-JsonFile -Path (Join-Path $PackageRoot 'VERSION.json') -Value ([ordered]@{
    schema_version = '1.0.0'; ecosystem_version = '1.2.26'; component = 'context_handoff'; version = '1.2.26'; engine_schema_version = '4.3.4'; source_commit = $Commit; source_tree_dirty = $false; source_payload_sha256 = $SourceDigest
  })
  Write-JsonFile -Path (Join-Path $PackageRoot 'SOURCE_ATTESTATION.json') -Value ([ordered]@{
    schema_version = '1.0.0'; ecosystem_version = '1.2.26'; component = 'context_handoff_source'; engine_schema_version = '4.3.4'; source_commit = $Commit; source_tree_dirty = $false; source_payload_sha256 = $SourceDigest; explicit_exclusions = $ExplicitExclusions; source_files = $SourceRecords
  })
...
try {
  ...
  $ArchivePath = Join-Path $TempRoot 'agentic-context-handoff-1.2.26.zip'
