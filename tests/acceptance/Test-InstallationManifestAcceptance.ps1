...
Assert-Equal -Actual $Version.package_version -Expected "1.2.25" -Message "Acceptance requires package 1.2.25 candidate."
Assert-Equal -Actual $Version.runtime_version -Expected "1.2.25" -Message "Acceptance requires runtime 1.2.25."
Assert-Equal -Actual $Version.companion_version -Expected "1.2.25" -Message "Acceptance requires Companion 1.2.25."
...
foreach ($InstallerContract in @(
  ...
  foreach ($ForbiddenLiteral in @('"1.2.6"', '"1.2.3"', '"1.2.9"', '"1.2.10"', '"1.2.11"', '"1.2.12"', '"1.2.13"', '"1.2.14"', '"1.2.15"', '"1.2.16"', '"1.2.17"', '"1.2.18"', '"1.2.19"', '"1.2.20"', '"1.2.21"', '"1.2.22"', '"1.2.23"', '"1.2.25"')) {
  ...
  if (!$SkipBash) {
    ...
    if ([string]::IsNullOrWhiteSpace([string]$BashExecutable)) {
      ...
      foreach ($GitArguments in @(
        ...
        $GitResult = Invoke-Capture -FilePath "git" -Arguments $GitArguments
