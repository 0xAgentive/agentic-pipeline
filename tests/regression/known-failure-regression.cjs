...
try{const p=json('tests/regression/KNOWN_FAILURE_PLAYBOOK_v1.2.8.json');check('KF-030',Array.isArray(p.cases)&&p.cases.length>=46,'playbook cases');}catch(e){check('KF-030',false,e.message)}
...
try{const d=text('scripts/windows/Test-DistributionIntegrity.ps1');check('KF-040',d.includes("ValidateSet('operational', 'strict')")&&d.includes('Distribution advisory warnings'),'operational/advisory gate separation');}catch(e){check('KF-040',false,e.message)}
